...
RiscvGenerator::choosePromotedSlots(const IRFunction& func) const {
    ...
    // Back edges approximate loop depth/hotness. Nested loops naturally receive
    // multiple +32 boosts, so matrix/graph inner-loop values rank above setup slots.
    ...
    // the backend reserves t6 as its scratch. Caller-saved registers need no
    // prologue/epilogue traffic, so use them for the hottest four slots first.
    if (!hasCall) {
        ...
        // a1..a7 that do not occur anywhere in the function IR; this excludes
        // incoming parameters and the a-register shuffles left by tail-recursion
        // conversion, while giving main/small leaf helpers up to seven extra
        // zero-save-cost locals. a0 remains reserved for the return value.
        for (int ai = 1; ai < 8 && ci < candidates.size(); ++ai) {
    ...
    // profitable for loop-hot values, but was actively harmful for one-shot
    // argument temporaries and small helpers. Require either loop evidence or
    // enough static accesses to amortize those two memory operations.
    static const char* savedRegs[] = {
        "s0", "s1", "s2", "s3", "s4", "s5",
        "s6", "s7", "s8", "s9", "s10", "s11"
    };
    size_t si = 0;
